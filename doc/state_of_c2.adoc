...
Maybe obviously, the closer we move to XTDB 1.x semantics, the more
work there will be, and the closer to stay to SQL semantics, the
larger the difference to XTDB 1.x it will have.
...
* The query engine won't work like XTDB 1.x in any case, and without a
...
=== Semantic differences from XTDB 1.x
...
XTDB 1.x relies heavily on Nippy and the Clojure type-system, so there
...
In XTDB 1.x semantics, this requires us to unwind all lists early on,
and then the engine will work closer to XTDB 1.x. With SQL semantics,
